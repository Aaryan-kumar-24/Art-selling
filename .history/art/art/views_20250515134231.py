...
def cart(request):
    ...
    if request.method == 'POST':
        ...
        # Save the artwork only if all required fields are present
        if selltitle and d and p and sellerartwork:
            sellerart = Upload(title=selltitle, discription=d, price=p, art=sellerartwork)
            sellerart.save()
            sellerdata = {"selltitle": selltitle, "d": d, "p": p, "sellerartwork": sellerartwork}
        else:
            print("Missing fields, not saving to database.")
