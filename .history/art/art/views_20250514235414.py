...
def cart(request):
    ...
    try:
        if request.method=='POST':
            ...
            p = request.POST.get('p')
             = request.POST.get('')
        sellerdata = {"artofseller":artofseller ,"sellprice":sellprice , "sellDescription":sellDescription , "selltitle":selltitle, "d":d, "p":p, "":}
...
price=p)
        ...
        data = {'phone': phone1,'email' :email1, 'medium':medium,'price':price,'image':image,'custom':custom,'phone': phonec,'email' :emailc, 'state':statec,'city':cityc,'address':addressc,'feedback':feedbackc , "artofseller":artofseller , "sellprice":sellprice ,"sellDescription":sellDescription , "d":d , "":, "p":p, "title":selltitle}
