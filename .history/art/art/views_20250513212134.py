...
def artworks(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if search!=None:
                myupload = Upload.objects.filter(title__icontains = search)
  

           
        return render(request, "all_artworks.html", {'artworks': final, 'myupload':myupload})
    except:
        pass
...
def header(request):
    ...
    try:
        if request.method == 'POST':
           search = request.POST.get('searches', '').strip().lower()
           if search:
                filtered_artworks = [
                art for art in artworks if search in art['title'].lower()
            ]

        
    except:
        pass
    return render(request, "header.html", {'artworks': filtered_artworks})
