...
def artworks(request):
    ...
    try:
        if request.method=='POST':
            ...
            sellerar = request.POST.get('sellerar')
...
def cart(request):
    ...
    try:
        if request.method=='POST':
            ...
            p = request.POST.get('p')
            sellerar = request.POST.get('sellerar')
        sellerdata = {"artofseller":artofseller ,"sellprice":sellprice , "sellDescription":sellDescription , "selltitle":selltitle, "d":d, "p":p, "sellerar":sellerar}
...
price=p)
        ...
        data = {'phone': phone1,'email' :email1, 'medium':medium,'price':price,'image':image,'custom':custom,'phone': phonec,'email' :emailc, 'state':statec,'city':cityc,'address':addressc,'feedback':feedbackc , "artofseller":artofseller , "sellprice":sellprice ,"sellDescription":sellDescription , "d":d , "sellerar":sellerar, "p":p, "title":selltitle}
